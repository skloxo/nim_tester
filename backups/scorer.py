...
def rank_category(model_score_map: dict) -> dict:
    ...
    elapsed_values = [v["avg_elapsed"] for v in model_score_map.values()
                      if v["avg_tps"] == 0 and v["avg_elapsed"] > 0]
    max_tps = max(tps_values) if tps_values else 0
    min_elapsed = min(elapsed_values) if elapsed_values else 0  # 越小越快

    scored = {}
    for mid, s in model_score_map.items():
        if s["avg_tps"] > 0 and max_tps > 0:
            speed_bonus = s["avg_tps"] / max_tps * 10
        elif s["avg_elapsed"] > 0 and min_elapsed > 0:
            # elapsed 越小越好：最快者得 10 分，其余按比例
